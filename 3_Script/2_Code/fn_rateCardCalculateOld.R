...
rateCardCalcuateOld <- function(rDataFolder,InvoiceDataOrderMapped, rateCard, Weight_different_Threshold, Absolute_weight_threshold, Total_Amount_Threshold){
    ...
    rateCard <- read.csv(file.path(inputFolder,"ReferenceData", paste0(rateCard,".csv")),
                         stringsAsFactors = FALSE, row.names = 1)
    h <- function(w) if( any( grepl( "In numericnumericConversion", w) ) ) invokeRestart( "muffleWarning" )
    ...
    InvoiceDataOrderMapped %<>%
        ...
        mutate(Final.Caluated.Weight=ifelse(!is.na(TotalSKUWeight) & TotalSKUWeight > 0 & 
                                                (Invoice.Chargeable.Weight > Weight_different_Threshold*TotalSKUWeight &
                                                     (Invoice.Chargeable.Weight > Absolute_weight_threshold |
                                                          Total.Amount > Total_Amount_Threshold)),
                                            ceiling(TotalSKUWeight),Final.Caluated.Weight)) %>%
        ...
        mutate(FinalAmount=ifelse(Variance<=Total_Amount_Variance_Threshold,Total.Amount,TotalAmount_cal))
